...
const AddEmployee = () => {
    ...
    const getEmpData = async (EmpId) => {
        try {
            const response = await fetch(`${baseURL}api/getInterview?id=${EmpId}`, {
                method: 'GET',
                headers: {
                    ...headers
                }
            ...
            setFatherName(result[0]?.fathers_name)
            setDesignation(result[0]?.role_id)
            setDepartment(result[0]?.department_id)
            setHiredBy(result[0]?.hired_by_employee_id)
            setEmpType(result[0]?.status)
        } catch (error) {
            console.log(error);
        }
    }
...
setIsloading(false)
    ...
    const renderPage = (num) => {
        switch (num) {
            ...
            case 2:
                return <AddEmployee_form2 locationOptions={locationOptions} floorData={floorData} hiredByData={hiredByData} supervisorData={supervisorData} head_employee_data={headEmployeeData} spl_key={designation} formSelect={form2Selects} formInput={form2Input} spl={spl_form2_selects} role_data={roleData} />
            case 3:
                return <AddEmployee_form3 formInput={form3Input} changeModeOfPay={changeModeOfPay} mode_of_pay={mode_of_pay} changeFineMgmt={changeFineMgmt} fine_mgmt={fine_mgmt} newFile={newFile} chanageEmpType={chanageEmpType} emp_type={emp_type} edit={true} data={data} photo={photo} changePhoto={changePhoto} isPhotoChanged={isPhotoChanged} changeDocument={changeDocument} isEdit={false} />
