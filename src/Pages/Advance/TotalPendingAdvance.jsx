...
const TotalPendingAdvance = () => {
  ...
  // Here is our data for tile in the page
  const fetchData=()=>{
    let getString = baseURL + "api/getAdvances?limit="+limit+"&offset="+offset+"&status='Approved'"
    if (date?.length>0){
    ...
    getString = baseURL + "api/getAdvances?"+"&status='Approved'"
    if (date?.length>0){
        let from_date=moment(date[0])
...
fetchData()
  ...
  useEffect(() => {
      ...
      let getString = baseURL + "api/getAdvances?limit="+limit+"&offset="+offset+"&status='Pending'"
      ...
      if(employeeFilter.employee_query!=''  &&employeeFilter.employee_query!==undefined){
        ...
        fetchAdvance({ url: getString }, listAdvance)
        getString = baseURL + "api/getAdvances?"+"&status='Pending'"
        if(employeeFilter.location_name!=''){
          getString+="location_name="+employeeFilter.location_name
        }
      if(employeeFilter.employee_query!='' &&employeeFilter.employee_query!==undefined){
  ...
  return (
    <React.Fragment>
      ...
      <Filter date={date} data={data} isdate={true} changeDate={changeDate} changeByDesignation={changeByDesignation} changeByEmployee={changeByEmployee} />
...
export default TotalPendingAdvance