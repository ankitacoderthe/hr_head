...
const PendingLeaves = () => {
  ...
  // Here is our data for tile in the page'
  const fetchData=()=>{
    let getString = url + "api/getLeaves?&limit="+limit+"&offset="+offset+"&status='Pending'"
    if (date?.length>0){
    ...
    getString = url + "api/getLeaves?"+"status='Pending'"
    if (date?.length>0){
      let from_date=moment(date[0])
...
if(employeeFilter.location_name!=''){
  ...
  return (
    <React.Fragment>
      <Heading heading={'Total Pending Leaves'} Btn_link={'/add_leave'} Btn={'Leave'} />
